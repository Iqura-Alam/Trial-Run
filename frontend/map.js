...
document.getElementById('locationInput').addEventListener('input', async (e) => {
  ...
  try {
    const res = await fetch(`https://trial-run-ibp5.onrender.com/api/institutes?q=${encodeURIComponent(query)}`);
...
document.getElementById('goBtn').addEventListener('click', async () => {
  ...
  const res = await fetch(`https://trial-run-ibp5.onrender.com/api/institutes?q=${encodeURIComponent(name)}`);
